...
attach_path     = GyG_constantes.recibos_temp          # "C:/Users/MColosso/Documents/GyG Recibos_temp/"
...
#    return f"{number:,.{num_decimals}f}".replace(',', 'x').replace('.', ',').replace('x', '.')
    return locale.format_string(f'%.{num_decimals}f', number, grouping=True)

def attachment_name(idx):
    return attach_path + attach_name.format(recibo=idx)
...
def sendmail(toaddr, subject, body, attachments=[]):
    ...
    try:
        ...
        error_msg  = os.path.normcase(error_msg) #.replace('\\', '/')
