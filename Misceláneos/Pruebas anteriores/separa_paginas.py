...
#  HISTÓRICO
#
#  Ok CORREGIR: Cambiar referencias de "df['Archivo']" a "df['Nro. Recibo']" para evitar problemas
#     de numerarión manual (corregido 28/06/2019)
#  Ok CORREGIR: En la hoja de cálculo '1.1. GyG Recibos.xlsm' se cambió el título de la columna 'B'
...
aux_folder     = 'temp/'
pdf_file       = GyG_constantes.pdf_GyG_Recibos       # '1.3. GyG Recibos.pdf'
patrón_pdfs    = GyG_constantes.patrón_pdfs           # 'GyG Recibo_%05d.pdf'
...
def separa_recibos():
    try:
        ...
        print('')
        sys.exit()    # do NOT use exit(), quit() or os.exit() to stop script execution
...
def copia_recibos_de_pago():
    ...
    print(f'Cargando la hoja de cálculo "{excel_workbook}"...')
...
if Ok:
    ...
    print(f'  -> renombrando desde "{receipt_number}" ...')
    renombra_recibos()
