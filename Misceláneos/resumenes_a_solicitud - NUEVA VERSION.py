...
# POR HACER:
# ----------
#  Ok Indicar la cantidad de vecinos seleccionados
#  Ok En el resumen de cuotas, ignorar los meses en los cuales no hay una cuota
#     definida (NaN)
#  Ok Revisar formas más eficientes para seleccionar las fechas de inicio y final:
...
#  X  ¿Cómo manejar la fecha real de inicio de una familia, para utilizar ésta como
...
#  Ok Seleccionar la impresión de resúmenes de dos posibles fuentes: Histórico (en
#     bolívares fuertes) o el archivo estándar (en bolívares soberanos)
#  Ok Generar resumen de saldos, tanto del archivo normal como del histórico
#  Ok Cambiar el cursor a HourGlass al iniciar las rutina ckHistorico_Seleccion() y
#     genera_resumenes() y devolverlo al estándar al finalizar
#  Ok Opción para desplegar o no el texto 'a solicitud de la parte interesada '
#     -- 12/03/2019
#  Ok Cambiar el texto del botón btAcepta a '', 'Genera resumen' o 'Genera resúmenes' si
...
#  Ok Enviar recibo de pago a múltiples destinatarios de correo electrónico
#     ^--> Usar la misma estrategia que en "envia_recibos.py": Colocar las diferentes
#          direcciones de correo separadas por una barra vertical:
#          me@mailserver.com | 0555-550.1234 | you@anothermailserver.com (06/05/2019)
...
pdf_file = 'GyG Resumen {:05d}.pdf'
pdf_path = 'C:/Users/MColosso/Google Drive/GyG Recibos/Resúmenes/'

excel_wb_historico = '1.1. GyG Recibos - SOLO CONSULTA.xlsm'
excel_wb_estandar  = '1.1. GyG Recibos.xlsm'
...
excel_vigilancia  = 'Vigilancia'
excel_resumen     = 'RESUMEN VIGILANCIA'   # <-- 'R.VIGILANCIA (reordenado)'
excel_saldos      = 'Saldos'
...
ini_file_path = 'resumenes_a_solicitud.ini'
...
def genera_resumenes():
    ...
    def genera_y_envia_resumen():
        ...
        if beneficiario_anterior != '':
            ...
            if genera_pdf(r_anterior, resumen, idx + ultimo_resumen):
                generados += 1
                enviado = envia_por_correo(r_anterior, idx + ultimo_resumen, email_o_celular_anterior)
                if enviado:
                    enviados += 1
    ...
    # proceso
    beneficiario_anterior = ''
    email_o_celular_anterior = ''   # Captura el último e-mail o celular
    r_anterior = None
    idx = 0
    generados = 0
    enviados = 0
    if despliega_saldos:
        for i in range(df.shape[0]):
            ...
            resumen.append([r['Fecha'].strftime('%d/%m/%Y'),
                            ...
                            '{:,.02f}'.format(r['Saldo'])])
            email_o_celular_anterior = r['E-mail o celular']
    else:
        for i in range(df.shape[0]):
            r = df.iloc[i]
            if r['Beneficiario'] != beneficiario_anterior:
...
#    print(f'       type of hoja_resumen = {type(hoja_resumen)}')
#    print(f'       dir(hoja_resumen.views) = {dir(hoja_resumen.views)}')
#    hoja_resumen.views.Pane(topLeftCell=f'A{celda_superior}')
#
# ----------------------------------------------------------------------------------------------
...
#   FOOTER1  Se genera dinámicamente en el cuerpo, con las cuotas vigentes durante el período solicitado
    ...
    EQUIPO_COBRANZA = '<font size=12><i>Equipo de Cobranza. Junta Directiva GyG</i></font>'
    ...
    filename = pdf_path + pdf_file.format(idx)
    ...
    FOOTER1 = '<font size=12>' + resumen_de_cuotas() + '</font>'
    # Genera la firma
    if ckCuotas.get() > 0:
        elements.append(Spacer(1, 16))
...
#    origen_de_la_solicitud = '' if despliega_saldos else 'a solicitud de la parte interesada '
    ...
    try:
        ...
        error_msg  = str(sys.exc_info()[1])
        error_msg  = error_msg.replace('\\', '/')
        actualiza_status(txStatus, f' <pdf>: <error>Error generando "{pdf_file.format(idx)}": {error_msg}')

    return pdf_generado


def envia_por_correo(r, idx, email_o_celular):

    if not envia_correos:
        return False   # El correo no fue enviado

    for send_to in decode_email_o_celular(email_o_celular):
        es_correo  = isinstance(send_to, list) or is_email(send_to)
        if not es_correo:
            actualiza_status(txStatus, f', <email>: <error>"{list_to_string(send_to)}" no es una dirección de correo válida')
            return False

        # Compose some of the basic message headers
        para   = dict()
        for enviar_a in send_to:
            para.update({enviar_a: r['Beneficiario']})
        ...
        anexo = pdf_path + pdf_file.format(idx)
        ...
        try:
            ...
            error_msg  = str(sys.exc_info()[1])
            error_msg  = error_msg.replace('\\', '/')
            actualiza_status(txStatus, f', <email>: <error>Error enviando "{pdf_file.format(idx)}": {error_msg}')
...
def resumen_de_cuotas():
    ...
    def rango_fechas(idx_inicial, idx_final, cuota, rango_final):
        punto_y_coma = ';' if not rango_final else ''
        precision = '' if consulta_historico else '.02f'
        cuota_ed = f"{cuota:,{precision}}".replace(',', 'x').replace('.', ',').replace('x', '.')
        if idx_inicial == idx_final:
            texto = f" {columnas[idx_inicial]:%B/%Y}, " + moneda + f" {cuota_ed}{punto_y_coma}"
        else:
            ...
            texto = f" {de}{columnas[idx_inicial]:%B{año_inicio}}" + \
                    f" {a_y} {columnas[idx_final]:%B/%Y}, " + moneda + \
                    f" {cuota_ed}{punto_y_coma}"
        return texto
    ...
    cuota = cuotas[idx]
    last_notnull = idx
    moneda = 'Bs.' if consulta_historico else 'Bs.S'
    txtCuotasMensuales = 'Durante este período, las mensualidades vigentes para el pago de la vigilancia ' + \
...
def genera_interfase():
    ...
    try:
        ...
        ventana_ppal.geometry(ini_file.read())
        ini_file.close()
    except:
...
#    ventana_ppal.bind("<Configure>", save_size)
    ...
    imgCheck = tk.PhotoImage(file='imagenes/check.gif')
    imgWarn  = tk.PhotoImage(file='imagenes/warn.gif')
    imgError = tk.PhotoImage(file='imagenes/error.gif')
    imgInfo  = tk.PhotoImage(file='imagenes/info.gif')
    imgStop  = tk.PhotoImage(file='imagenes/stop.gif')
    imgPdf   = tk.PhotoImage(file='imagenes/pdf.gif')
    imgEmail = tk.PhotoImage(file='imagenes/mail.gif')
...
def finaliza_app():
    with open(ini_file_path, 'w') as conf:
        conf.write(ventana_ppal.geometry())
...
def abre_hoja_de_recibos_de_pago():
    global df_resumen, df_cuotas

    print(f'Cargando hoja de cálculo "{excel_workbook}"...')
    df_resumen = read_excel(excel_workbook, sheet_name=excel_resumen)
    df_cuotas  = df_resumen[df_resumen['Beneficiario'] == 'CUOTAS MENSUALES']
    df_resumen = df_resumen[['Beneficiario', 'Dirección']]   # En la columna de Beneficiarios
    #                           , 'F.Desde', 'F.Hasta'       # está la lista de vecinos
