...
"""
    POR HACER
    -   Convertir el proceso en una rutina que pueda ser usada externamente
    -   
...
def copia_recibo(r):
    global input_path, filename, output_path, output_file

    input_path      = os.path.normpath(input_path)
    output_path     = os.path.normpath(output_path)

    beneficiario    = r['Beneficiario'].replace('Familia ', 'Fam. ')
    nro_recibo      = int(r['Nro. Recibo'])
    codRecibo       = '{:05d}'.format(nro_recibo)
